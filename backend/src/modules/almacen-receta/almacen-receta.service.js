const Repo = require('./almacen-receta.repository');
const { beginTransaction } = require('../../config/database');
const { AppError } = require('../../middleware/errorHandler');

class AlmacenRecetaService {
    ...
    async getProductosIntermediosActivos() {
        const result = await Repo.getProductosIntermediosActivos();
        if (!result || result.length === 0) {
    ...
    async updateEstadoRecetaAlmacen(id, { id_planta_receta_almacen = 0, id_almacen_producto_intermedio = 0, estado = 0 } = {}) {
        let result = false;
...
module.exports = new AlmacenRecetaService();
