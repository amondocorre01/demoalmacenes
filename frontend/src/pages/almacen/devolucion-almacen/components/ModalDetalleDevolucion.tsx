...
import { DevolucionItem } from '../services/useDevolucionAlmacen';

interface ModalDetalleDevolucionProps {
  open: boolean;
  onClose: () => void;
  item: DevolucionItem | null;
}

export const ModalDetalleDevolucion: React.FC<ModalDetalleDevolucionProps> = ({
  open,
  onClose,
  item,
}) => {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h2 className="text-lg font-black text-on-surface uppercase tracking-tight leading-none font-headline">
                DEVOLUCIÓN #{item.ID_DOCUMENTO_DEVOLUCION || item.ID_DEVOLUCION || item.ID_PLANTA_ALMACEN_DOCUMENTO || '-'}
              </h2>
            </div>
          </div>
          <IconButton
        ...
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
              ...
              <div className="space-y-1">
                ...
                <span className="font-bold text-zinc-600 dark:text-zinc-400 block">
                  ...
                  {formatTime(item.FECHA_REGISTRO, item.HORA_REGISTRO)
                    ? `• ${formatTime(item.FECHA_REGISTRO, item.HORA_REGISTRO)}`
                    : ''}
                </span>
              </div>
          ...
          <div className="p-4 bg-surface-variant/40 dark:bg-zinc-850/40 rounded-2xl border border-outline-variant/60 dark:border-zinc-800 space-y-3">
            <div className="flex items-center gap-2 text-primary border-b border-outline-variant/40 dark:border-zinc-800 pb-2">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest font-headline">
                PRODUCTOS DEVUELTOS ({detalleList.length})
              </span>
            </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-zinc-50/70 dark:bg-zinc-850/60 border-b border-zinc-200 dark:border-zinc-800">
                      ...
                      <td className="w-[120px] px-1 py-2.5 text-[9px] font-black uppercase tracking-widest text-zinc-400 dark:text-zinc-500 text-center">
                        Devuelto
                      </td>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800/60">
                    {detalleList.length === 0 ? (
                      <tr>
                        <td colSpan={5} className="py-6 text-center text-zinc-400 text-xs font-bold">
                          No hay productos detallados en este registro.
                        </td>
                      </tr>
                    ) : (
                      detalleList.map((prod: any, idx) => (
                        <tr
                          key={idx}
                        ...
                        >
                          ...
                          <td className="px-3 py-2">
                            <span className="text-xs font-black text-zinc-900 dark:text-zinc-100 uppercase tracking-tight">
                              {prod.PRODUCTO_DETALLE || `Producto #${prod.ID_PRODUCTO || prod.ID_PRODUCTO_DETALLE}`}
                            </span>
                          ...
                          <td className="px-3 py-2 text-center">
                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-lg text-xs font-black bg-primary/10 text-primary border border-primary/20">
                              {prod.CANTIDAD_DEVUELTA ?? prod.CANTIDAD ?? 0}
                            </span>
                          </td>
                          <td className="px-3 py-2 text-center text-[10px] font-black uppercase tracking-wider text-zinc-500 dark:text-zinc-400">
                            {prod.UNIDAD_MEDIDA || prod.UNIDAD_MEDIDA_A || 'UNIDAD'}
                          </td>
                          <td className="px-3 py-2 text-center text-[10px] font-black uppercase tracking-wider text-zinc-500 dark:text-zinc-400">
                            {formatDate(prod.FECHA_VENCIMIENTO)}
                          </td>
                          <td className="w-[120px] px-1 py-2 text-center text-[9px] font-black text-zinc-500 dark:text-zinc-400">
                            {prod.DEVUELTO?.length > 0 && (
                              <div className="space-y-1">
                                {prod.DEVUELTO.map((item: any, index: number) => (
                                  <div
                                    key={index}
                                  ...
                                  >
                                    ...
                                    <span className="text-[8px] text-zinc-400 dark:text-zinc-500">
                                      Venc: {formatDate(item.FECHA_VENCIMIENTO)}
                                    </span>
                                  </div>
                                ))}
                              </div>
