...
import { AlmacenItem } from '../services/useReposicionProductosVencidos';
...
export const ModalSeleccionarAlmacenes: React.FC<ModalSeleccionarAlmacenesProps> = ({
  ...
  const handleToggleWarehouse = (item: AlmacenItem) => {
    if (isAllSelected) {
      // Si estaba en "Todos", desmarcamos "Todos" y dejamos solo el clickeado
    ...
    // Si seleccionó todos los individuales disponibles manualmente, o no quedó ninguno
    if (updated.length === 0) {
      setTempSelected([ALL_OPTION]);
    } else {
  ...
  // Cantidad seleccionada para visualización
  const selectedCount = isAllSelected ? warehouses.length : tempSelected.length;
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center px-1">
              ...
              <span className="text-[10px] font-bold text-primary font-body">
                {isAllSelected
                  ? 'Todos seleccionados'
                  : `${selectedCount} de ${warehouses.length} seleccionados`}
              </span>
            </div>
