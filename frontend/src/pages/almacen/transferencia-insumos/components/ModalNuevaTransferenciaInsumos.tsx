...
interface ModalNuevaTransferenciaInsumosProps {
  open: boolean;
  onClose: () => void;
  userWarehouses: any[];
  allWarehouses: any[];
...
interface TransferInsumoItemState {
  id: number;
  product: ProductoInsumoStock;
  cantidad: string;
}

export const ModalNuevaTransferenciaInsumos: React.FC<ModalNuevaTransferenciaInsumosProps> = ({
  open,
  onClose,
  userWarehouses,
  allWarehouses,
  ...
  const { loadApiGetStockInsumos, loadApiRegistrarTransferencia } = useTransferenciaAlmacenInsumosServices();
  ...
  const [availableStock, setAvailableStock] = useState<ProductoInsumoStock[]>([]);
  const [selectedStockItem, setSelectedStockItem] = useState<ProductoInsumoStock | null>(null);
  const [transferItems, setTransferItems] = useState<TransferInsumoItemState[]>([]);
  ...
  const fetchStock = async (idAlmacen: number) => {
    ...
    const res = await loadApiGetStockInsumos(idAlmacen);
  ...
  const handleAddProduct = () => {
    if (!selectedStockItem) {
      showAlert.warning('Seleccionar Insumo', 'Por favor seleccione un insumo del listado.');
      return;
    }

    if (transferItems.some((item) => item.product.ID_PRODUCTO_DETALLE === selectedStockItem.ID_PRODUCTO_DETALLE)) {
      showAlert.warning('Insumo duplicado', 'El insumo seleccionado ya está en el detalle.');
      return;
    }

    if (selectedStockItem.STOCK <= 0) {
      showAlert.warning('Sin stock', 'El insumo seleccionado no cuenta con stock disponible en el almacén origen.');
      return;
    }
  ...
  const handleSaveTransfer = async () => {
    ...
    if (transferItems.length === 0) {
      showAlert.warning('Sin insumos', 'Debe agregar al menos un insumo a la transferencia.');
      return;
    }
    ...
    if (invalidItem) {
      const prodName = invalidItem.product.NOMBRE || invalidItem.product.PRODUCTO || 'el insumo';
      showAlert.warning(
        'Cantidad inválida',
        `Ingrese una cantidad válida mayor a 0 para ${prodName}.`
      );
      return;
    }
    ...
    try {
      const payload = {
        ...
        productos: transferItems.map((item) => ({
          ...
          id_producto_detalle: item.product.ID_PRODUCTO_DETALLE,
          cantidad_adecuacion: item.product.CANTIDAD_ADECUACION || 1,
          cantidad: parseFloat(item.cantidad),
          id_unidad_medida: item.product.ID_UNIDAD_MEDIDA || 0,
          producto: item.product.NOMBRE || item.product.PRODUCTO || '',
        })),
      };

      const res = await loadApiRegistrarTransferencia(payload);
      if (res && res.success) {
        showAlert.success('Transferencia Exitosa', res.message || 'Se registró la transferencia de insumos correctamente.');
        onSaveSuccess();
        onClose();
      } else {
          ...
          <div className="p-4 bg-surface-variant/40 dark:bg-zinc-850/40 rounded-2xl border border-outline-variant/60 dark:border-zinc-800 space-y-3">
            ...
            <div className="flex flex-col sm:flex-row items-end gap-3">
              <div className="w-full sm:flex-1 space-y-2">
                <label className="block text-[10px] font-black uppercase tracking-widest text-zinc-400 dark:text-zinc-500 ml-1">
                  Buscar Insumo en Stock ({availableStock.length} disponibles)
                </label>
                <Autocomplete
                  options={availableStock}
                  getOptionLabel={(o) => `${o.NOMBRE || o.PRODUCTO} (Stock: ${o.STOCK ?? 0} ${o.UNIDAD_MEDIDA || o.UNIDAD_MEDIDA_A || ''})`}
                  value={selectedStockItem}
                  onChange={(_, val) => setSelectedStockItem(val)}
                  loading={isLoadingStock}
                  disabled={!sourceWarehouse || isLoadingStock}
                  fullWidth
                  noOptionsText={isLoadingStock ? 'Cargando stock...' : 'No hay insumos con stock en este almacén'}
                  sx={{
                    '& .MuiOutlinedInput-root': {
                      borderRadius: '15px',
                      backgroundColor: 'var(--input-bg, var(--surface))',
                  ...
                  renderInput={(params) => (
                    <TextField {...params} variant="outlined" size="small" placeholder="ESCRIBA O SELECCIONE INSUMO..." />
                  )}
                />
              </div>
          ...
          <div className="p-4 bg-surface-variant/40 dark:bg-zinc-850/40 rounded-2xl border border-outline-variant/60 dark:border-zinc-800 space-y-3">
            <div className="flex justify-between items-center border-b border-outline-variant/40 dark:border-zinc-800 pb-2">
              <div className="flex items-center gap-2 text-primary">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest font-headline">
                  DETALLE DE INSUMOS A TRANSFERIR ({transferItems.length})
                </span>
              </div>
            </div>
                  ...
                  <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800/60">
                    {transferItems.length === 0 ? (
                      <tr>
                        <td colSpan={6} className="py-8 text-center text-zinc-400 dark:text-zinc-500">
                          ...
                          <span className="text-[10px] font-black uppercase tracking-wider">
                            Utilice el selector arriba para añadir insumos a transferir
                          </span>
                        </td>
                      </tr>
                    ) : (
                      transferItems.map((item, idx) => {
                        ...
                        const prodName = item.product.NOMBRE || item.product.PRODUCTO || 'Insumo';
                        return (
                          <tr key={item.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-850/40 transition-colors">
                            ...
                            <td className="px-3 py-2 text-center text-[10px] font-black uppercase tracking-wider text-zinc-500 dark:text-zinc-400">
                              {item.product.UNIDAD_MEDIDA || item.product.UNIDAD_MEDIDA_A || 'UNIDAD'}
                            </td>
