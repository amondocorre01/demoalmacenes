/**
 * ReposicionUsuarioAlmacen.tsx
 * ─────────────────────────────────────────────────────────────
 * 1. Propósito de la vista:
 *    Gestión, auditoría y control de reposición de productos e insumos por usuario en almacenes.
 *    Permite visualizar los registros de reposición generados, consultar el stock disponible,
 *    registrar nuevas reposiciones y asignar o modificar el usuario responsable que asume el
 *    costo del producto directamente en la tabla o mediante modal.
 *
 * 2. APIs Utilizadas:
 *    - GET  /v1/inventario/declaracion/almacenes?id_planta_almacen=0 (loadApiGetAlmacenes - Almacenes permitidos)
 *    - GET  /v1/inventario/rep-desp/reposiciones                   (loadApiGetReposiciones - Historial con id_estado: 15 estático)
 *    - GET  /v1/inventario/rep-desp/productos-stock                (loadApiGetProductosStock - Stock en almacén)
 *    - POST /v1/inventario/rep-desp/registrar-reposicion           (loadApiRegistrarReposicion - Registrar reposición)
 *    - POST /v1/inventario/rep-desp/asignar-responsable-reposicion (loadApiAsignarResponsable - Asignar usuario responsable)
 *
 * 3. Controles Clave y Reglas de Negocio:
 *    - Parámetro estático obligatorio id_estado: '15' en la consulta de reposiciones.
 *    - Selector de almacenes con soporte para selección múltiple o global mediante modal especializado.
 *    - Filtro por rango de fechas (Fecha Inicio y Fecha Fin) con reseteo reactivo de la lista de datos.
 *    - Resumen métrico centralizado e interactivo (Total Registros, Con Responsable, Sin Asignar).
 *    - Asignación en línea de responsable con Autocomplete directamente en cada fila de la tabla.
 *    - Modales estandarizados según AGENTS.md ubicados en components/.
 */
...
import { ModalRegistrarReposicion } from './components/ModalRegistrarReposicion';
...
export const ReposicionUsuarioAlmacen: React.FC = () => {
    const {
        loadApiGetAlmacenes,
    ...
    const [itemsList, setItemsList] = useState<ReposicionItem[]>([]);
    const [isLoading, setIsLoading] = useState<boolean>(false);
    const [isAssigningId, setIsAssigningId] = useState<number | null>(null);

    // Control para restricción de fecha (opcional)
    const [controlarRestriccionFecha] = useState<boolean>(false);
    ...
    const [modalAssignOpen, setModalAssignOpen] = useState<boolean>(false);
    const [selectedItemForModal, setSelectedItemForModal] = useState<ReposicionItem | null>(null);
    ...
    // Consultar reposiciones (id_estado: 15 estático)
    const handleFetchReposiciones = useCallback(async () => {
        setIsLoading(true);
        try {
            ...
            const data = await loadApiGetReposiciones({
                almacenes: almacenesParam,
                fecha_inicio: fi,
                fecha_fin: ff,
                id_estado: '15', // Estático obligatorio
            });
    ...
    }, [startDate, endDate, selectedWarehouses, loadApiGetReposiciones]);
    ...
    const handleInlineAssignResponsible = async (
        ...
        if (!newUser) return;
        const idReposicion =
            item.ID_PLANTA_REPOSICION_ALMACEN ||
            item.ID_REPOSICION_ALMACEN ||
            0;

        if (!idReposicion) {
            showAlert.error('Error', 'No se pudo identificar el registro de reposición.');
            return;
        }

        setIsAssigningId(Number(idReposicion));
        const res = await loadApiAsignarResponsable(
            Number(idReposicion),
            Number(newUser.ID_USUARIO)
        );
        ...
        if (res && res.success) {
            setItemsList((prev) =>
                prev.map((row) => {
                    const rowId =
                        row.ID_PLANTA_REPOSICION_ALMACEN ||
                        row.ID_REPOSICION_ALMACEN;
                    if (rowId === idReposicion) {
                        return {
                            ...row,
    ...
    const filteredItems = useMemo(() => {
        return itemsList.filter((item) => {
            ...
            return prod.includes(term) || alm.includes(term) || user.includes(term) || detalle.includes(term);
        });
    ...
    const assignedCount = useMemo(() => {
        return itemsList.filter(
            (item) =>
                (item.ID_USUARIO_ASUMIDO && item.ID_USUARIO_ASUMIDO > 0) ||
                (item.USUARIO_ASUMIDO &&
                    item.USUARIO_ASUMIDO.trim() !== '' &&
                    item.USUARIO_ASUMIDO !== 'Sin asignar')
        ).length;
    }, [itemsList]);
    ...
    const exportColumns: TableColumnConfig[] = [
        ...
        { header: 'Unidad', width: 65, align: 'center' },
    ...
    const getExportData = () => {
        return filteredItems.map((item, idx) => {
            ...
            const unit = item.UNIDAD_MEDIDA || item.MEDIDA || 'UND';
    ...
    const handleExportExcel = () => {
        ...
        exportTableToExcel({
            title: 'REPOSICIÓN POR USUARIO EN ALMACÉN',
            subtitle: `RANGO: ${fi} AL ${ff}`,
            filename: `reposicion_usuario_${dayjs().format('YYYYMMDD_HHmm')}`,
    ...
    return (
        <div className="max-w-[1600px] mx-auto w-full space-y-2 p-2 sm:p-0 animate-in fade-in duration-300 font-body">
            <LoadingOverlay show={isLoading} message="Cargando reposiciones de almacén..." />

            {/* ── Encabezado Principal ── */}
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 className="text-2xl font-black text-on-surface uppercase tracking-tight font-headline">
                        REPOSICIÓN POR USUARIO EN ALMACÉN
                    </h1>
                    <p className="text-[10px] font-black text-on-surface-variant mt-0 font-body">
                        Auditoría de reposición, control de mermas y asignación de responsabilidades
                    </p>
                </div>
                ...
                <Grid container spacing={1.5} alignItems="flex-end">
                    ...
                    <Grid size={{ xs: 12, sm: 6, md: 1 }} sx={{ paddingTop: { xs: '0px', sm: '22px', md: '22px' } }}>
                        <div className="flex justify-start pb-0.5">
                            <button
                                type="button"
                                onClick={handleFetchReposiciones}
                                title="Buscar Reposiciones"
                            ...
                            {paginatedItems.length === 0 ? (
                                <tr>
                                    <td colSpan={10} className="py-8 text-center text-zinc-400 dark:text-zinc-500">
                                        ...
                                        <span className="text-xs font-bold uppercase tracking-wider">
                                            {isLoading ? 'Cargando registros...' : 'No se encontraron reposiciones'}
                                        </span>
                                    </td>
                                </tr>
                            ) : (
                                paginatedItems.map((item, idx) => {
                                    ...
                                    const productName =
                                        ...
                                        (item.NOMBRE_DETALLE ? item.NOMBRE_DETALLE : '-');
                                        ...
                                        >
                                            ...
                                            <td className="w-48 max-w-[200px] px-2 py-1.5 uppercase">
                                                <span className="block text-[10px] font-bold text-zinc-900 dark:text-zinc-100 leading-tight">
                                                    {item.NOMBRE_DETALLE || productName}
                                                </span>
                                                {item.NOMBRE_DETALLE && productName && item.NOMBRE_DETALLE !== productName && (
                                            ...
                                            <td className="w-1 px-2 py-1.5 font-bold text-right text-zinc-700 dark:text-zinc-300 whitespace-nowrap text-[11px]">
                                                Bs.{' '}
                                                {Number(
                                                    item.PRECIO_CONSUMO_INTERNO ?? item.PRECIO ?? 0
                                            ...
                                            <td className="w-1 px-2 py-1.5 font-black text-right text-zinc-900 dark:text-zinc-100 whitespace-nowrap text-[11px]">
                                                Bs.{' '}
                                                {Number(item.TOTAL_ASUMIDO ?? item.TOTAL ?? 0).toLocaleString(
                                                    'es-BO',
                                                    {
                                                        minimumFractionDigits: 2,
                                                        maximumFractionDigits: 2,
                                            ...
                                            {/* Usuario Responsable (Inline Autocomplete) */}
                                            <td className="px-3 py-1">
                                                <div className="relative min-w-[220px] max-w-[280px]">
                                                    <Autocomplete
                                                            ...
                                                            (item.USUARIOS || []).find(
                                                                (u) =>
                                                                    (item.ID_USUARIO_ASUMIDO &&
                                                                        u.ID_USUARIO === item.ID_USUARIO_ASUMIDO) ||
                                                                    (u.NOMBRE_COMPLETO &&
                                                                        u.NOMBRE_COMPLETO.toUpperCase() ===
                                                                        (item.USUARIO_ASUMIDO || item.USUARIO || '').toUpperCase()) ||
                                                                    (u.USUARIO &&
                                                                        u.USUARIO.toUpperCase() ===
                                                                        (item.USUARIO_ASUMIDO || item.USUARIO || '').toUpperCase())
                                                            ) || null
                                                        }
                                                        onChange={(_, newValue) => {
                                                        ...
                                                        disabled={!perm.allowed || isAssigningId === Number(idReposicion)}
                                                    ...
                                                    />
                                                    {isAssigningId === Number(idReposicion) && (
            ...
            {/* ── Modal Nueva Reposición ── */}
            <ModalRegistrarReposicion
                open={modalNewOpen}
                onClose={() => setModalNewOpen(false)}
                onSuccess={handleFetchReposiciones}
            ...
            <ModalAsignarResponsable
                ...
                reposicionItem={selectedItemForModal}
                onSuccess={handleFetchReposiciones}
            />
...
export default ReposicionUsuarioAlmacen;
